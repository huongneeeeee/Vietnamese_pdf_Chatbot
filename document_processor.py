...
class DocumentDatabaseManager:
    ...
    # [MỚI] Hàm chọn loader dựa trên đuôi file
    ...
    def process_document(self, document):
        text_splitter = RecursiveCharacterTextSplitter(
            separators=["\n\n", "\n", " ", ".", "!", "?", ""],
            chunk_size=512,
            chunk_overlap=128, # Giảm overlap chút để tiết kiệm
            length_function=len
        )
    ...
    def load_existing_db(self, file_name):
        # File name now creates a folder based on its clear name
    ...
    def update_db(self, file_path):
        ...
        # [MỚI] Sử dụng hàm get_loader đa năng
        ...
        if chunks:
            ...
            # Lưu DB với tên thư mục là tên file đã làm sạch
    ...
    def delete_file_data(self, file_name):
        """Xóa dữ liệu vector và hash của một file cụ thể"""
        existing_hashes = self.load_existing_hashes()
        
        # Tìm hash tương ứng để xóa
        hash_to_remove = None
...
class ContextRetriever:
    ...
    def expand_context(self, file_name_clean, context, num_words=150):
        ...
        if not all_text: return context # Trả về context gốc nếu không tìm thấy file

        # Tìm đoạn text trong file gốc (xử lý đơn giản)
        # Lưu ý: context từ FAISS có thể bị cắt, nên tìm kiếm chính xác đôi khi thất bại
        # Ở đây ta dùng logic đơn giản: nếu tìm thấy thì mở rộng
        idx = all_text.find(context)
        if idx != -1:
            start = max(0, idx - num_words * 5) # Ước lượng 1 từ = 5 ký tự
    ...
    def get_file_name(self, metadata):
        ...
        # Trả về tên file sạch (không dấu) để map với file txt
